...
class Model(Generic[T]):
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        # Task 7.9
        for formula in formulas:
            free_variables = tuple(formula.free_variables())

            def evaluate_for_all_assignments(index: int,
                                             assignment: Mapping[str, T]) -> bool:
                ...
                for value in self.universe:
                    ...
                    if not evaluate_for_all_assignments(index + 1,
                                                        new_assignment):
                        return False
                return True

            if not evaluate_for_all_assignments(0, frozendict()):
                return False
        return True
